...
- Pre-process outputs live in `cache/` and are mirrored into the **PostgreSQL primary database** (earlier drafts referenced `data/index.db` as a file DB; keep that as historical only). Cached artifacts remain the rebuild source of truth; PostgreSQL + pgvector is the single operational store going forward. Content-hash changes invalidate cache-side artifacts and near-duplicate pairs for that image and trigger incremental recompute; primary rows remain unchanged for auditability.
